...
const cards = [
  { icon: Briefcase, label: "Role", value: "Full-Stack Software Developer" },
  { icon: MapPin, label: "Location", value: "Kampala, Uganda" },
  { icon: Clock, label: "Available", value: "Mon–Fri, 8:30am – 4:30pm" },
  { icon: GraduationCap, label: "Education", value: "BSc CS, Makerere (Evening)" },
]
...
export default function About() {
  return (
    <section id="about" className="py-28 px-6 relative">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/3 w-64 h-64 bg-violet-600/8 rounded-full blur-3xl" />
      </div>
      <div className="max-w-6xl mx-auto relative z-10">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          >
            {cards.map(({ icon: Icon, label, value }) => (
              <div
                key={label}
                className="bg-white/5 border border-white/10 rounded-2xl p-5 hover:border-violet-500/50 hover:bg-violet-500/5 transition-all duration-300"
              >
                <Icon size={20} className="text-violet-400 mb-3" />
